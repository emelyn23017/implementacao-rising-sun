...
################################################################################
## GLOBAL VARIABLE AND GLOBAL SETTINGS
...
def figure_save(figure_name, tight_layout=True, figure_extension="eps", resolution=300):
    ...
    logger.info("Saving figure as ", figure_name.replace(" ", "_"))
    if tight_layout:
        plt.tight_layout()
...
def write_material_peak_properties(
    ...
    for name in names:
        decompositions[name] = rising_sun_envelope_method(
            parameters,
            hyperparameters,
            name,
            N_split_before,
...
def normalization(materials, names, normalizeheight=False):
    ...
    if normalizeheight:
        ...
        for name in names:
            X[name], normalization_heights[name] = normalize(
                X[name], norm="max", axis=1, return_norm=True
            )
...
def oscillation_function(interval, f, full_computation=True):
    ...
    if full_computation:
        for l in range(1, L):
            oscillation[0, l] = np.max(
                [
                    np.max(f[0, k : k + l + 1]) - np.min(f[0, k : k + l + 1])
                    for k in range(L - l)
                ]
            )
        ...
        oscil_number = np.max(
            [
                np.max(f[0, k : k + 1 + 1]) - np.min(f[0, k : k + 1 + 1])
                for k in range(L - 1)
            ]
        )
...
def plateau_detection(v, L_threshold):
    ...
    where_plts_start_and_length = np.concatenate(
        [[0], where_plts_start_and_length[0], [0]]
    ).reshape(1, -1)
    ...
    for i in range(1, L_threshold - 1):
        ...
        is_there = [
            ...
            if str(where_plts_start_and_length[0, j : j + sequence.shape[1]])
            == str(sequence.squeeze())
        ]
        if is_there != []:
            for k in is_there:
                where_plts_start_and_length[0, k : k + sequence.shape[1]] = np.zeros(
                    [1, sequence.shape[1]]
                )  ## erase that small string
    ...
    for i in range(Len_where):
        ...
        if (
            where_plts_start_and_length[0, i + 1] - where_plts_start_and_length[0, i]
            == 1
        ):
            locations = np.concatenate(
                (locations, np.array([i], np.int16, ndmin=2)), axis=1
            )  # subtract 1, because the vector was augmented by 0 in the extremes
...
def padded_spectra(E, materials, names):
    ...
    for name in names:
        ...
        padded_material[name] = np.reshape(
            np.pad(material_aux, (0, stopping[name]), "edge"), (1, -1)
        )
...
def calculate_derivatives(
    csv_file_path, column_x, column_y, output_1st_deriv, output_2nd_deriv
):
    """
...
def decay_rate(
    ...
    if decay_rate_type == "min_max":
        decay_factor = (
            np.max(xanes_now[0, start:]) - np.min(xanes_now[0, start:])
        ) / omega_oscillation[0, -1]
        ...
        dist_estimated = max(
            int(max(decay_factor, lambda_d) * np.average(distances)), 2
        )
    ...
    ###################################
        ...
        predicted_distance = np.exp(
            lin_reg_dist.predict(np.reshape(interval[-1, 0], (-1, 1)))
        )
        dist_estimated = max(
            int(
                ((lambda_d * np.exp(-(j - 1)) + (j - 1) / 2) / j)
        ...
        E_for_pred = np.reshape(
            E_now[0, start : start + int(2 * dist_estimated)], (-1, 1)
        )
        xanes_for_pred = np.reshape(
            xanes_now[0, start : start + int(2 * dist_estimated)], (-1, 1)
        )
        ...
        decay_plot = {
            "range_energy_pred": E_for_pred,
            "range_xanes_pred": pred,
            "error": error,
        }
        ...
        height_threshold = (
            decay_factor * vector_height_threshold[0]
        )  # np.average(vector_height_threshold)
...
def estimate_distance(parameters, hyperparameters, name, rising_sun, oscillation):
    ...
    what_matters = np.where(
        heights >= min(lambda_h_find_1st * oscillation, 0.5)
    )  ### Lmabda_h ==5 before
    ...
    min_xanes_now = max(
        np.min(new_xanes_now), rising_sun[0, 0]
    )  ## WE do this in case the function starts in a valley
...
def find_peak(
    ...
    if len(locations) == 0:
        ...
        return (locations[0], interval[0, locations[0]], heights[0], num_iterations)
...
def find_first_peak(
    ...
    min_location, max_location = estimate_distance(
        parameters, hyperparameters, name, rising_sun, oscill_threshold
    )
    tdist_peak = np.array(
        [int(lambda_d_shrink_1st * (max_location - min_location))], ndmin=1
    )  ## IT WAS 1/2 before

    # Find peak locations:
    iteration_flag = 0
    tpeak_loc0, tpeak_energy0, tpeak_height0, tnum_itera0 = find_peak(
        E_now,
        rising_sun,
        first_peak_threshold,
        tdist_peak[-1],
    ...
    tdist_peak = np.array(
        [int(lambda_d_shrink_1st * (tpeak_loc0 - min_location))]
    )  ## IT WAS 1/2 before
...
def print_brkpt_properties(j, peak_loc, peak_height, dist_peak, jumps):
    ...
    ----------------
    """
    logger.info("\t Peak " + str(j) + " located at: " + str(peak_loc))
    logger.info("\t Peak height:" + str(peak_height))
    logger.info("\t Distance between successive crests and peaks: ", dist_peak, "\n")
...
def hidden_breakpoint(xanes_now, left, middle, right, j):
    ...
    if (
        xanes_now[0, middle] == M
    ):  ## In this case there is no need for the hidden peak trick
        return middle
...
def rising_sun_envelope_method(
    ...
    vector_peak_loc, vector_height_threshold = np.array([], dtype=np.int16), np.array(
        [], dtype=np.float16
    )
    ...
    for j in range(max(N_split_after, 1)):
        if j == 0:
            ...
            if forward == False:
                ...
                vector_height_threshold = np.append(
                    vector_height_threshold, height_threshold[str(0)]
                )
                continue

            if printing:
                logger.info("************************************** \n")
                logger.info("\t Going for " + str(j) + "th breakpoint. \n")
            ...
            height_threshold["0"] = (
                int(oscillation[0, -1] / (init_oscil_par * critical_threshold)) + 1
            ) * critical_threshold
            ...
            vector_height_threshold = np.append(
                vector_height_threshold, height_threshold[str(j)]
            )

            if printing:
                print_brkpt_properties(
                    j, peak_loc[str(j)], peak_height[str(j)], dist_peak, jumps
                )
        ...
        # secondary cases - for which we have to use a maximal function
            ...
            f = (
                np.array(xanes_now[0, start:], ndmin=2) - height_normalizer
            )  ## you always normalize
            rising_sun_f = rising_sun(f)

            # Update the definition of the height threshold
            decay_factor, temp, height_threshold[str(j)], _ = decay_rate(
                parameters,
                hyperparameters,
                name,
                oscillation,
            ...
            tpeak_loc, tpeak_energy, tpeak_height, tnum_itera = find_peak(
                interval,
                f,
                height_threshold[str(j)],
                temp,
                0,
                iteration_flag,
                hyperparameters,
            ...
            if (tpeak_loc == None) or (
                tpeak_loc + start >= stop
            ):  ### Then you can stop here!!!
                if printing:
                    logger.info(
                        "\n ONLY "
                        + str(number_split_after_peak)
                        + " SPLITTINGS WERE POSSIBLE!\n"
                    )
                break
            ...
            if j >= 2:
                ...
                if (move == "middle_step") or (peak_loc[str(j - 1)] == p):
                    ...
                    decay_factor, _, height_threshold[str(j)], _ = decay_rate(
                        parameters,
                        hyperparameters,
                        name,
                        oscillation,
                    ...
                    jumps[-1] = np.abs(
                        xanes_now[0, peak_loc[str(j - 1)]]
                        - xanes_now[0, peak_loc[str(j - 2)]]
                    )
            ...
            dist_peak = np.append(
                dist_peak, np.array([int(peak_loc[str(j)] - peak_loc[str(j - 1)])])
            )

            # unnormalize peak height
            peak_height[str(j)] = xanes_now[
                0, peak_loc[str(j)]
            ]  # ==peak_height[str(j)]+height_normalizer

            # Book keeping
            jumps = np.append(
                jumps,
                np.abs(
                    xanes_now[0, peak_loc[str(j)]] - xanes_now[0, peak_loc[str(j - 1)]]
                ),
            )
    ...
    oscil_jump_ratio = {}
    x, y = np.array(E_now[0, peak_loc["0"] : stop], ndmin=2), np.array(
        xanes_now[0, peak_loc["0"] : stop], ndmin=2
    )
    ...
    for i in range(1, number_split_after_peak + flag_oscillation):
        x, y = np.array(E_now[0, peak_loc[str(i)] :], ndmin=2), np.array(
            xanes_now[0, peak_loc[str(i)] :], ndmin=2
        )
        _, _, amount_oscil = oscillation_function(x, y, full_computation=False)
    ...
    if N_split_before == 0:
        if printing:
            logger.info(
                "\n ONLY "
                + str(number_split_after_peak + number_split_before_peak)
                + " SPLITTINGS \
        ...
        decomposition = {
            ...
            "number_iterations": num_itera,
            "distance_between_peaks": dist_peak,
            "oscillation": oscillation,
            "height_threshold_evolution": height_threshold,
            "jumps": jumps,
    ...
    if printing:
        logger.info("************************************** \n")
        logger.info("\t STARTING REVERSE PASSING, ON THE LEFT SIDE\n")
    ...
    reverse_E, reverse_xanes, reverse_stop = small_padded_spectra(
        chopped_E, chopped_xanes
    )

    # Things that we are going to use in the REVERSE PASSING
    # ... parameters
    reverse_N_split_after = (
        N_split_before + 1
    )  # add 1 because we are not looking for 0 anymore
    ...
    reverse_search_conditions = {
        "move": "middle_step",
        "printing": False,
        "polite_guess": reverse_polite_guess,
        "forward": False,
    }
    ##################################################################
    ...
    reverse_decomposition = rising_sun_envelope_method(
        reverse_parameters,
        reverse_hyperparameters,
        name,
        reverse_N_split_before,
        reverse_N_split_after,
        reverse_search_conditions,
    )
    ...
    reverse_number_splittings_before = reverse_decomposition[
        "number_of_splittings_before"
    ]
    reverse_number_splittings_after = reverse_decomposition[
        "number_of_splittings_after"
    ]
...
def plot_marked_peak_spectrum(
    ...
    loc=4,
    title="XANES spectrum with marked peaks",
):
    """
    ...
    for name in names:
        ...
        plt.plot(
            E_now[0, 0:stop].T,
            xanes_now[0, 0:stop].T,
            label=name + ",#Breakpoints: " + str(N),
            color="C" + str(i % 10),
            lw=3,
            linestyle="-",
            alpha=0.7,
...
for xdi in arquivos:
    logger.info(f"Processando o arquivo: {xdi}")
    ...
    if data[0] == "Transmission":
        if CZDS_utils.XASNormalization.xas_type(data[1]) == "EXAFS":
            enorm, mu_norm = CZDS_utils.XASNormalization.EXAFS_normalization(
                data[1], data[4], debug=False
            )
            plt.scatter(enorm, mu_norm)
            plt.show()
        else:
            enorm, mu_norm = CZDS_utils.XASNormalization.XANES_normalization(
                data[1], data[4], debug=False
            )
    ...
    calculate_derivatives(
        ...
        "energy",
        filename,
        "XANES-derivatives/1st_deriv.csv",
        "XANES-derivatives/2nd_deriv.csv",
    )
    ...
    for i in range(1, data_derivatives.shape[1]):
        ...
        raw_materials_1st_derivatives[name] = np.reshape(
            data_val_derivatives[:, i], (1, -1)
        )
    ...
    for i in range(1, data_derivatives.shape[1]):
        name = names_1st_derivatives[i - 1]
        E_derivatives[name] = np.reshape(data_val_derivatives[:, 0], (1, -1))
        raw_materials_2nd_derivatives[name] = np.reshape(
            data_val_derivatives[:, i], (1, -1)
        )

    materials, shift_height, normalizer_shift = normalization(
        raw_materials, names, normalizeheight=True
    )
    ...
    for name in {names[0]}:
        ...
        plt.plot(
            ...
            xanes_now_unpad.T,
            color="C0",
            label=str(names[i]) + "non-normalized",
            lw=4,
            linestyle="-",
        )
        plt.plot(
            ...
            xanes_now.T,
            color="C1",
            label=str(names[i]) + "normalized and padded",
            lw=4,
            linestyle="-",
        )
    ...
    for name in names:
        ...
        plt.plot(
            ...
            rising_sun_f[name].T,
            color="C" + str(i % 10),
            label="$\mathscr{R}_{\mu}(\cdot)$ of " + str(name),
            marker="o",
            markersize=4,
            lw=3,
            linestyle=":",
    ...
    for name in names:
        ...
        valley_of_shadows[name] = (
            rising_sun_f[name] - xanes_now
        )  # this function will always be nonnegative, thanks to (1)
        plt.plot(
            E_now.T,
            valley_of_shadows[name].T,
            color="C" + str(i % 10),
            label=name,
            lw=4,
            linestyle="-",
    ...
    for name in {names[0]}:
        ...
        plt.plot(
            E_now[0, 0 : stopping[name]].T,
            xanes_now[0, 0 : stopping[name]].T,
            color="C" + str(i % 10),
            lw=4,
            linestyle="--",
            label=r"$\mu(\cdot)$: " + str(name),
        )
        plt.plot(
            E_now[0, 0 : stopping[name]].T,
            rising_sun_f[name][0, 0 : stopping[name]].T,
            color="red",
            lw=4,
            linestyle="-",
            label=r"$\mathcal{R}_{\mu}(\cdot)$ :" + str(name),
            markersize=2,
        )
        plt.plot(
            E_now[0, 0 : stopping[name]].T,
            valley_of_shadows[name][0, 0 : stopping[name]].T,
            color="blue",
            linestyle="-.",
            lw=4,
            label=r"$\mathcal{V}_{\mu}(\cdot)$ : " + str(name),
        )
    ...
    for name in names:
        ...
        (interval[name], oscillation[name], estimatE_now) = oscillation_function(
            E_now, xanes_now
        )
        new_label = name + ", noise:" + str(round(estimatE_now, 4))
        plt.plot(
            interval[name].T, oscillation[name].T, linestyle="-", lw=4, label=new_label
        )
    ...
    for name in {names[0]}:
        ...
        (interval_rising, oscillation_rising, _) = oscillation_function(
            E_now, rising_sun_f[name]
        )
        ax1.plot(
            interval_rising.T,
            oscillation_rising.T,
            linestyle="-",
            lw=2,
            label="$\omega^{\mathscr{R}_{\mu}}$: " + name,
            color="gray",
        )

        ax1.title.set_text(
            "Oscillation function of $\omega^{\mu}$ and $\omega^{\mathscr{R}_{\mu}}$"
        )
        ...
        new_label = name + ", noise:" + str(round(estimatE_now, 4))
        ax2.plot(
            interval[name][0, 50:200].T,
            oscillation[name][0, 50:200].T,
            linestyle="-",
            lw=2,
            label="$\omega^{\mu}$: " + name,
            color="r",
        ...
        ax2.plot(
            interval_rising[0, 50:200].T,
            oscillation_rising[0, 50:200].T,
            linestyle="-",
            lw=2,
            label="$\omega^{\mathscr{R}_{\mu}}$: " + name,
            color="gray",
    ...
    decompositions = write_material_peak_properties(
        parameters,
        hyperparameters,
        search_conditions,
        names,
    ...
    for name in names:
        logger.info(
            name,
            "has",
            decompositions[name]["number_of_splittings_before"],
            " splittings before, and",
            decompositions[name]["number_of_splittings_after"],
            "splittings after\n",
        )
    ...
    plot_marked_peak_spectrum(
        ...
        {names[0]},
        decompositions,
        "raw_XANES_with_marked_peaks_average",
        plot_type="raw",
    )
